...
export const useGetRecognitionOverview = () => {
  return useQuery<RecognitionOverview>({
    ...
    queryFn: async () => {
      return apiClient.get("/recognition/overview").then((res) => res.data);
    },
  });
};
...
export const useStartRecognition = () => {
  ...
  return useMutation({
    mutationFn: (meetingCode: string) => {
      return apiClient
        .patch(`/meetings/start-recognition/${meetingCode}`)
        .then((res) => res.data);
    },
    onSuccess: () => {
...
export const useStopRecognition = () => {
  ...
  return useMutation({
    mutationFn: (meetingCode: string) => {
      return apiClient
        .patch(`/meetings/stop-recognition/${meetingCode}`)
        .then((res) => res.data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries();
...
export const useSelectRecognitionModel = () => {
  ...
  return useMutation({
    mutationFn: ({
      meetingCode,
      RecognitionModel,
    }: SelectRecognitionModelArgs) => {
      return apiClient
        .patch(`/meetings/select-recognition-model/${meetingCode}`, {
          RecognitionModel,
        })
        .then((res) => res.data);
    },
